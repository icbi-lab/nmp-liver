...
# %%
figwidth = min(max(adata.obs["sample_id"].unique().size * 0.5, 2), 20)

# %%
fig, ax = plt.subplots(1, 1, figsize=(figwidth, 5))
sc.pl.violin(
    adata, "total_counts", groupby="sample_id", rotation=90, log=True, cut=0, ax=ax
)

# %%
fig, ax = plt.subplots(1, 1, figsize=(figwidth, 5))
sc.pl.violin(adata, "pct_counts_mito", groupby="sample_id", rotation=90, ax=ax)
